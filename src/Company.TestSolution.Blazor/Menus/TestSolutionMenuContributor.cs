...
using Company.TestSolution.Permissions;
...
namespace Company.TestSolution.Blazor.Menus;

public class TestSolutionMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public TestSolutionMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<TestSolutionResource>();

        context.Menu.Items.Insert(
            0,
            new ApplicationMenuItem(
                TestSolutionMenus.Home,
                l["Menu:Home"],
                "/",
                icon: "fas fa-home",
                order: 1
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                TestSolutionMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(TestSolutionPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                TestSolutionMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(TestSolutionPermissions.Dashboard.Tenant)
        );
